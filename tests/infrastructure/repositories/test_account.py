...
def test_create_account(account_repository, user):
    account_data = {
        "user_id": user.id,
...
def test_create_account_commit(db, account_repository, user):
    account_data = {
        "user_id": user.id,
        "daily_withdrawal_limit": 500.0,
    }
...
def test_get_by_person_id_returns_account_when_found(
    ...
    created_account = create_account(user_id=user.id)
...
def test_get_by_args_returns_account_when_found(
    ...
    retrieved_account = account_repository.get_by_args(user_id=user.id)
...
def test_get_accounts_info_returns_correct_total_and_count(
    ...
    create_account(amount=100, user_id=user.id)

    create_account(amount=200, user_id=user.id)
...
def test_filter_returns_accounts_matching_criteria(
    ...
    filtered_accounts = account_repository.filter(user_id=user.id)
